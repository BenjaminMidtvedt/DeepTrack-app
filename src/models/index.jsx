                ...
                src={
                    typeof props.sources[index] === "string"
                        ...
                        : "data:image/bmp;base64, " +
                          props.sources[index].toString("base64")
                }></img>
            {props.sources.length > 1 ? (
                <div style={{ marginLeft: 10, marginRight: 10 }}>
                    <Slider
                        ...
                        value={index}
                        onChange={(e, v) => setIndex(v)}></Slider>
                </div>
            ) : null}
        </div>
    );
        ...
        return (
            <div
                ...
                <div style={{ flexGrow: 1, margin: 10 }}>
                    <ResultDisplay
                        title="Prediction"
                        src={(this.props.predictions[frame] || [])[index] || ""}
                    />
                </div>
            ...
            <div
                ...
                {this.props.test_results && this.props.test_results.scores ? (
                    <div
                        ...
                        {Object.entries(
                            this.props.test_results.scores || {}
                        ).map(([key, value]) => (
...
class LossVisualisation extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (this.variables.length === 0) {
            ...
            newProps.properties.forEach((proplist) => {
                ...
                Object.entries(vars).forEach(([key, value]) => {
                    ...
                    if (not_equal.length === 0) {
                        frame[key] = v0;
                        if (!variable_names.includes(key))
            ...
            let reduced_names = variable_names.filter((name) => {
                let framewise = variables
                    .map((framevars) => framevars[name])
    ...
    updatePath() {
        ...
        if (variable === "epoch") {
            ...
            svg.select(".valline")
                .transition()
                .attr("d", validation_line(loss));
            svg.select(".vallineback")
                .transition()
                .attr("d", validation_backline(loss));
        } else {
    ...
    render() {
        return (
            <div style={{ display: "flex", flexDirection: "row" }}>
                <svg
                    id={"svgloss" + this.props.id}
                    style={{ width: 630, flexShrink: 0 }}></svg>
                <form>
                    <FormControl
                        style={{ width: 200, padding: 10 }}
                        variant="outlined">
                        <InputLabel htmlFor="select-variable">
                            Variable
                        </InputLabel>
                        <MySelect
                            native
            ...
            {visualiserIndex == 1 || visualiserIndex == 2 ? (
                <div
                    ...
                    <IconButton
                        onClick={() => {
                            ...
                            const interval = Math.min(
                                10000 / (props.predictions.length + 1),
                                100
                            );
                    ...
                    <Slider
                        ...
                        onChange={(e, value) => setFrame(value)}></Slider>
                </div>
            ) : null}
        </div>
    );
                ...
                <div style={{ width: "100%" }}>
                    <div style={{ padding: 30 }}>
                        {jobQueue.map((job, i) => (
                            <div
                                key={job.id}
                                className={classes.form + " background--20"}>
                                ...
                                <Button
                                    ...
                                    onClick={(e) => {
                                        const current_store = store.getState()
                                            .undoable.present.items;

                                        let model_index = undefined;

                                        store.dispatch(GROUPBEGIN());
                                        const new_store = current_store.map(
                                            (f, idx) => {
                                                if (
                                                    f &&
                                                    f.name === "Network" &&
                                                    f.class === "featureGroup"
                                                ) {
                                                    const new_f = { ...f };
                                                    new_f.items = [];
                                                    model_index = idx;
                        ...
                        <form className={classes.form + " background--20"}>
                            ...
                            <FormControl className={classes.textField}>
                                ...
                                <BootstrapInput
                                    ...
                                    id="batch_size"
                                    type="number"
                                    defaultValue="16"></BootstrapInput>
                            </FormControl>
                            ...
                            <FormControl className={classes.textField}>
                                ...
                                <BootstrapInput
                                    id="number_of_epochs"
                                    type="number"
                                    defaultValue="100"></BootstrapInput>
                            </FormControl>
                            ...
                            <FormControl className={classes.textField}>
                                ...
                                <BootstrapInput
                                    id="min_data_size"
                                    type="number"
                                    defaultValue="200"></BootstrapInput>
                            </FormControl>
                            ...
                            <FormControl className={classes.textField}>
                                ...
                                <BootstrapInput
                                    id="max_data_size"
                                    type="number"
                                    defaultValue="1000"></BootstrapInput>
                            </FormControl>
                            ...
                            <FormControl className={classes.textField}>
                                ...
                                <BootstrapInput
                                    id="validation_size"
                                    type="number"
                                    defaultValue="64"></BootstrapInput>
                            </FormControl>
                            ...
                            <FormControl className={classes.textField}>
                                ...
                                <BootstrapInput
                                    id="patience"
                                    type="number"
                                    defaultValue="10"></BootstrapInput>
                            </FormControl>
                            ...
                            <FormControl className={classes.textField}>
                                <Button
                                    ...
                                    onClick={() => {
                                        ...
                                        config.batch_size = document.getElementById(
                                            "batch_size"
                                        ).value;
                                        config.epochs = document.getElementById(
                                            "number_of_epochs"
                                        ).value;
                                        config.validation_set_size = document.getElementById(
                                            "validation_size"
                                        ).value;
                                        config.validation_freq = document.getElementById(
                                            "validation_frequency"
                                        ).value;
                                        config.min_data_size = document.getElementById(
                                            "min_data_size"
                                        ).value;
                                        config.max_data_size = document.getElementById(
                                            "max_data_size"
                                        ).value;
                                        config.patience = document.getElementById(
                                            "patience"
                                        ).value;
                                        config.test_set_size = document.getElementById(
                                            "test_set_size"
                                        ).value;
                                        config.name = "Placeholder name";
                                        Python.enqueueTraining(
                                            config,
                                            (err, res) => {
                                                if (res) {
                                                    this.setState({
