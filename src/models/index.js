        ...
        return (
            <div style={{display: "flex", flexDirection:"row", alignItems:"center"}}>
                ...
                <IconButton style={{borderRadius: 5, padding:20, transform: "rotate(90deg)", marginLeft:-90, marginRight: -90}} onClick={()=>{
                        ...
                        this.setState({index: (this.state.index - 1 + this.props.predictions[0].length) % this.props.predictions[0].length})
                    }}>
                ...
                <IconButton style={{borderRadius: 5, padding:20, transform: "rotate(-90deg)", marginLeft:-90, marginRight: -90}} onClick={()=>{
                        if (!this.props.predictions || this.props.predictions.length === 0) return
                        this.setState({index: (this.state.index + 1) % this.props.predictions[0].length})
                    }}>
                    <div className="chevron" ></div>
                </IconButton>
            </div>
...
class StatusVisualisation extends React.Component {
    ...
    updatePath() { 
        ...
        const self = this
        const svg = d3.select("#svg"+this.props.id)
        svg.selectAll("g").select(".barpath").each(function(d) {self.__oldData__.set(this, d)})
        ...
        bars.select(".barpath").attr("fill", (d) => d.color)
            .transition()
            .duration(1000)
            .attrTween('d', function(d) {
                const old_data = self.__oldData__.get(this) 
                const old_count = old_data ? old_data.count : 0
                const iter = d3.interpolate(2 * Math.PI * old_count / d.maxCount, 2 * Math.PI * d.count / d.maxCount)
                return (t) => {d.endAngle = iter(t); return arc(d)}
            })
    }
...
class LossVisualisation extends React.Component {
    ...
    updatePath() { 
        ...
        let X = variable === "epoch" ? Array(loss.length).fill(0).map((a, i) => i + 1) : this.variables.map((v) => v[variable])
        ...
        const additional_metrics = include_val ? [loss.map(s=> s["val_" + metric])] : validations.map(v => v.map(v => v[metric]))
                ...
                <form>
                    ...
                    <FormControl style={{width:200}} variant="outlined">
                        ...
                        <MySelect native inputProps={{"id":"select-variable", "name": "Variable"}} label="Variable" value={this.state.variable} onChange={(e) => {this.setState({variable: e.target.value})}}>
                            ...
                            {this.props.validations.length > 0 ? this.variable_names.map(key => (
        ...
        <div>
            ...
            {visualiserIndex == 1 || visualiserIndex == 2 ?
                <div style={{display:"flex", flexDirection:"row", alignItems:"center", justifyContent:"center"}}>
                    ...
                    <Slider value={frame} min={0} max={props.predictions.length - 1 || 0} onChange={(e, value) => setFrame(value)}></Slider>
...
class PredictVisualiser extends React.Component {
    ...
    render() {
        ...
        return (
                ...
                <div
                    style={{width: 800, height:400, display:"flex", flexDirection:"row"}}
                    onDragOver = {(e)=>{
                            ...
                            <div key={job.id} className={classes.form}>
                                            <VisualisationPlayer {...job}/>
                                                <Button
                                                        ...
                                                        onClick ={(e) => {
                                                            ...
                                                            dialog.showSaveDialog({
                                                                defaultPath:job.id,
                                                                filters:[
                                                                  {name: 'Hierarchical Data Format (.h5)', extensions:['h5']}
                                                                ]
                                                              }).then(({filePath}) => {
                                                                  if (filePath) {
                                                                    fs.copyFile(path.join(appPath, "/tmp/models/", job.id + ".h5"), filePath, (err) => {
                                                ...
                                                <Button
                                                        ...
                                                        color="primary"
                                                        onClick ={(e) => {
                                                            store.dispatch({type: "SET_STATE", present: job})
                                                        }}>
                                                    Load configuration
                                                </Button>
                                                <Button
                                                        ...
                                                        onClick ={(e) => {
                                                            ...
                                                            store.dispatch({type:"SET_STATE", present: {items: new_store}})
                                                            store.dispatch(addItem(model_index, {
                                                                name: "LoadModel",
                                                                class: "feature",
                                                                key: "models",
                                                                type: "LoadModel"
