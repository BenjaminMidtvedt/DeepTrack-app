...
class MyReactTextareaAutocomplete extends React.Component<TextareaProps, TextareaState> {
    ...
    componentDidUpdate({ separators: oldSeparators, value: oldValue }: TextareaProps) {
        ...
        const update =
            (this.caretPosition.start !== false && this.caretPosition.start !== selectionStart) ||
    ...
    _handleChange(e) {
        ...
        const textToReplace =
            valueItems.length > 0 ? [lengthSum - valueItems[currentTargetIndex].length, lengthSum] : null;
        ...
        if (valueItems.length > 1 || areaStartPosition > 0) {
            ...
            Object.entries(this.props.tree).forEach(([key, value]) => {
                if (value._suggestionData && value._suggestionData.class !== 'feature') {
                    selection[key] = value;
                }
            });
        }
    ...
    _handleCaretChange(e) {
        ...
        const { top: newTop, left: newLeft } = getCaretCoordinates(e.target, selection);
    ...
    _createRegExp() {
        this.separatorExp = new RegExp('(?:' + this.props.separators.join('|') + ')', 'gm');
