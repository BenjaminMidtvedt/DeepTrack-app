            ...
            <div className="property">
                <Tooltip
                    ...
                    title={
                        ...
                        item.descriptions[item.name].length
                                    ...
                                    <IconButton
                                        ...
                                        <div
                                            className={
                                                "text-button " +
                                                (this.state.V
                                                    ? "text-button-selected text-button-active"
                                                    : "")
                                            }>
                                            V
                                        </div>
                    ...
                    <Collapse in={item.expand}>
                        <div className="collapsed">
                            {fields.map(({ field, label, placeholder }) =>
                                field === "S" ||
                                (this.props.SVTL[field] &&
                                    (this.state[field] ||
                                        !this.props.SVTL.S)) ? (
                                    <div
                                        key={field}
                                        className={parentType + "--90"}>
...
export class Feature extends React.Component {
    ...
    _update = throttle(() => {
        if (this._ref && this._ref.current) {
            this._ref.current.style.borderBottom = this.borderBottom
                ? `dashed 2px white`
                : ``;
            this._ref.current.style.borderTop = this.borderTop
                ? `dashed 2px white`
                : ``;
        }
    });
            ...
            Python.getFeatureProperties(item.type, (err, obj) => {
                if (obj) {
                    Object.entries(obj).forEach((value) => {
                        ...
                        if (description) {
                            this.property_descriptions[description[0]] = [
                                description[1],
                                description[2],
                            ];
                        }
        ...
        return (
            <div
                ...
                <div
                    ...
                    className={
                        "header-wrap grabbable accent-border " +
                        item.key +
                        "--90"
                    }
                    onClick={() => actions.toggleExpand(item.index)}>
                    <div
                        className={
                            "expand-wrap " +
                            (item.expand ? " expand-open" : "expand-closed")
                        }>
                        <ExpandMore
                            style={{ height: 24 }}
...
class FeaturePicker extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (!this.props.open && newProps.open) {
            ...
            custom_features = custom_features.filter((file) =>
                file.endsWith(this.props.extension || ".dtf")
            );
            custom_features = custom_features.map((file) =>
    ...
    render() {
        ...
        return (
            <Dialog
                onClose={handleClose}
                open={open}
                PaperProps={{ style: { position: "absolute", left: "150px" } }}>
                <List>
    ...
    _update() {
        if (this._ref && this._ref.current) {
            if (this.draggingOver && this.props.item.items.length === 0) {
                this._ref.current.style.backgroundColor =
    ...
    render() {
        ...
        return (
            <div
                ...
                <div
                    className={
                        this.props.parent && this.props.parent.key + "--90"
                    }>
                    <div
                ...
                <div className={"sidebar-item-noscroll"}>
                    <Collapse in={this.state.isopen}>
                        <div className="collapsed">
                            {to_list(item.items, item.index, this.props.SVTL)}
                        </div>
                    </Collapse>
                </div>
            </div>
        ...
        <div className="sidebar-item-wrapper">
            <div
                ...
                <div
                    ...
                    className={
                        "sidebar-icon-wrap " +
                        (expand ? " sidebar-icon-open" : " sidebar-icon-closed")
                    }>
...
const actions = {
    ...
    setValue: (index, field, value) =>
        store.dispatch(setValue(index, field, value)),
    removeItem: (index) => store.dispatch(removeItem(index)),
    dropItem: (index, target, child, position) =>
        store.dispatch(dropItem(index, target, child, position)),
    REDO: () => store.dispatch(REDO()),
    UNDO: () => store.dispatch(UNDO()),
    GROUPBEGIN: () => store.dispatch(GROUPBEGIN()),
    GROUPEND: () => store.dispatch(GROUPEND()),
    injectItems: (index, target, items, position) =>
        store.dispatch(injectItems(index, target, items, position)),
};
            ...
            <div id="dialog-container">
                ...
                <div id="dialog-image">
                    <img
                        src={
                            ...
                            (props.progressSource
                                ? props.progressSource.toString("base64")
                                : "")
                        }
                    />
                </div>
                <div id="dialog-progress">
                    <LinearProgress
                        variant="determinate"
                        value={
                            (props.itemsSaved / props.itemsRequested) * 100
                        }></LinearProgress>
                </div>
            </div>
            <DialogActions>
